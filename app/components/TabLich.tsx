...
export default function TabLich({
  ...
  soDienThoai, setSoDienThoai, theLoai, setTheLoai, theLoaiKhac, setTheLoaiKhac,
  goiChup, setGoiChup, giaTien, setGiaTien, formatTienInput, themHoacSuaLich,
  resetForm, timNgay, setTimNgay, tuKhoa, setTuKhoa, lichTheoNgay,
  laAdmin, capNhatTrangThai, suaLich, xoaLich
}: any) {
  
  // HÀM TẠO TIN NHẮN ZALO NHẮC LỊCH
  ...
  return (
    <>
      <div className="bg-white rounded-lg shadow p-4 mb-6">
        <h2 className="text-xl font-bold mb-4">{dangSua ? "Sửa lịch chụp" : "Thêm lịch chụp"}</h2>
        <div className="grid gap-3">
          ...
          <select value={theLoai} onChange={(e) => { setTheLoai(e.target.value); if (e.target.value !== "Khác") setTheLoaiKhac(""); }} className="border p-2 rounded">
          ...
          {theLoai === "Khác" && <input type="text" placeholder="Nhập thể loại mới" value={theLoaiKhac} onChange={(e) => setTheLoaiKhac(e.target.value)} className="border p-2 rounded" />}
          <input type="text" placeholder="Gói chụp" value={goiChup} onChange={(e) => setGoiChup(e.target.value)} className="border p-2 rounded" />
          
          <div>
            <div className="relative">
              <input type="text" inputMode="numeric" placeholder="Giá tiền" value={giaTien} onChange={(e) => setGiaTien(formatTienInput(e.target.value))} className="border p-2 rounded w-full pr-10" />
              <span className="absolute right-3 top-2 text-gray-500">đ</span>
            </div>
            {giaTien && <div className="text-sm text-green-600 mt-1">Giá trị: {giaTien}đ</div>}
          </div>

          <div className="flex gap-2">
            <button onClick={themHoacSuaLich} className="bg-blue-600 text-white p-2 rounded flex-1">{dangSua ? "Lưu thay đổi" : "Thêm lịch"}</button>
            {dangSua && <button onClick={resetForm} className="bg-gray-500 text-white p-2 rounded">Hủy</button>}
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-6">
        ...
        <div className="grid gap-3 md:grid-cols-3">
          ...
          <button onClick={() => { setTimNgay(""); setTuKhoa(""); }} className="bg-gray-500 text-white px-4 py-2 rounded">Xem tất cả</button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow p-4">
        {Object.entries(lichTheoNgay).sort(([a], [b]) => a.localeCompare(b)).map(([ngay, dsLich]: any) => (
            <div key={ngay} className="mb-6">
              <h2 className="text-xl font-bold mb-3">📅 {ngay.split("-").reverse().join("/")} ({dsLich.length} lịch)</h2>
              <div className="space-y-2">
                {[...dsLich].sort((a, b) => a.gio.localeCompare(b.gio)).map((item) => (
                    <div key={item.id} className="border rounded p-3 flex flex-col md:flex-row md:justify-between md:items-center gap-3">
                      <div>
                        ...
                        <div className="mt-2">
                          <select value={item.trangThai || "Chưa liên hệ"} onChange={(e) => capNhatTrangThai(item.id, e.target.value)} className="border p-1 rounded text-sm">
                      ...
                      <div className="flex gap-2 flex-wrap">
                        {/* NÚT COPY ZALO THẦN THÁNH */}
                        <button onClick={() => copyZaloLich(item)} className="bg-[#0068FF] text-white px-3 py-1 rounded">
                          💬 Zalo
                        </button>
                        
                        <button onClick={() => suaLich(item)} className="bg-yellow-500 text-white px-3 py-1 rounded">Sửa</button>
                        {laAdmin && <button onClick={() => xoaLich(item.id)} className="bg-red-500 text-white px-3 py-1 rounded">Xóa</button>}
                      </div>
                    </div>
                  ))}
              </div>
