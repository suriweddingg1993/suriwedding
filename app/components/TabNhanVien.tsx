    ...
    <div className="pb-24 px-2 pt-4">
      ...
      <div className="bg-blue-50 border border-blue-200 p-4 rounded-xl mb-6 shadow-sm flex gap-3 items-start">
        ...
        <div>
          ...
          <p className="text-xs text-blue-700 font-medium">1. Vào trang quản trị Firebase Authentication tạo tài khoản (Email & Mật khẩu).<br/>2. Copy dải mã <b className="bg-blue-200 px-1 rounded">UID</b> của tài khoản vừa tạo.<br/>3. Dán UID và điền các thông tin bên dưới để hoàn tất hồ sơ.</p>
        </div>
      </div>

      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-8">
        <h3 className="font-bold text-gray-800 mb-4 border-b border-gray-100 pb-2">{dangSuaNhanVien ? "✏️ Cập nhật hồ sơ nhân viên" : "✨ Thêm hồ sơ nhân viên mới"}</h3>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="md:col-span-2">
            ...
            <select 
              ...
              disabled={dangSuaNhanVien !== null && emailNhanVien === "dangngocan93@gmail.com"} // KHÓA HẠ QUYỀN ADMIN GỐC
          ...
          <div className="md:col-span-2 mt-2">
            <button onClick={taoHoSoNhanVien} className={`w-full py-3.5 rounded-xl font-bold text-white shadow-lg transition-transform active:scale-[0.98] ${dangSuaNhanVien ? "bg-yellow-500 hover:bg-yellow-600 shadow-yellow-200/50" : "bg-blue-600 hover:bg-blue-700 shadow-blue-200/50"}`}>
              {dangSuaNhanVien ? "💾 LƯU CẬP NHẬT" : "✨ TẠO HỒ SƠ MỚI"}
            </button>
            {dangSuaNhanVien && (
               <div className="text-center mt-3"><button onClick={() => window.location.reload()} className="text-sm text-gray-500 font-medium hover:text-gray-700 underline">Hủy chỉnh sửa</button></div>
            )}
          </div>
        </div>
      </div>
          ...
          <div key={tk.id} className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm hover:shadow-md hover:border-blue-200 transition-all">
            <div className="flex justify-between items-start mb-3 border-b border-gray-100 pb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className={`text-[10px] font-bold px-2 py-0.5 rounded w-fit mt-1 uppercase ${tk.role === "admin" ? "bg-red-50 text-red-600 border border-red-100" : "bg-blue-50 text-blue-600 border border-blue-100"}`}>{tk.role === "admin" ? "Quản lý / Admin" : "Nhân viên"}</div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-gray-600 font-medium mb-3">
              ...
              <div className="flex items-center gap-2 bg-slate-50 p-2 rounded-lg">
                <span>📞</span> 
                {tk.soDienThoai ? (
                  <><a href={`tel:${tk.soDienThoai}`} className="text-blue-600 hover:underline">{tk.soDienThoai}</a><NutCopy textCanCopy={tk.soDienThoai} /></>
            ...
            {laAdmin && (
              <div className="flex justify-between items-center bg-green-50 border border-green-100 rounded-xl p-3 mt-2">
                <div className="flex flex-col"><span className="text-[10px] font-bold text-green-700 uppercase">Lương cứng</span><span className="font-black text-green-800">{formatTienInput(String(tk.luongCung || 0))}đ</span></div>
                <div className="h-6 w-px bg-green-200"></div>
                <div className="flex flex-col text-right"><span className="text-[10px] font-bold text-green-700 uppercase">Thưởng chuyên cần</span><span className="font-black text-green-800">+{formatTienInput(String(tk.thuongChuyenCan || 0))}đ</span></div>
              </div>
            )}
          </div>
        ))}
